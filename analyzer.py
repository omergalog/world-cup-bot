...
ז. גזירת התוצאה המדויקת - אסור ברירת מחדל (כמו 2-1 לכל משחק)! חשב את מספר השערים של כל צד בנפרד מהנתונים:
   • הערך כמה שערים כל קבוצה צפויה לכבוש לפי: ה-xG ההתקפי שלה, איכות/חולשת ההגנה של היריבה, פצועים בהתקפה/הגנה, וכושר ההבקעה האחרון.
   • התאם את התוצאה לפרופיל המשחק: שתי התקפות חזקות + הגנות פתוחות → תוצאה גבוהה (3-2, 3-1, 2-2). משחק הגנתי/גוש סגור → תוצאה נמוכה (1-0, 0-0, 2-0). פייבוריטית מוחצת מול הגנה חלשה → רחב (3-0, 4-0, 4-1).
   • שאל את עצמך: האם הספרה הזו באמת נגזרת מהנתונים, או שאני פולט תוצאה נוחה? אם כל המשחקים יוצאים אותו דבר - עצור ונתח מחדש.
   • 2-1 ותיקו הם תוצאות לגיטימיות - אבל רק כשהפרופיל באמת מתאים להן, לא כברירת מחדל.
